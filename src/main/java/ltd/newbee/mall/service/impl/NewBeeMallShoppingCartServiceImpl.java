...
public class NewBeeMallShoppingCartServiceImpl implements NewBeeMallShoppingCartService {
    ...
    @Autowired
    private GoodsCategoryMapper goodsCategoryMapper1;
        ...
        if (!CollectionUtils.isEmpty(newBeeMallShoppingCartItems)) {
            ...
            for (NewBeeMallShoppingCartItem newBeeMallShoppingCartItem : newBeeMallShoppingCartItems) {
                ...
                if (newBeeMallGoodsMap.containsKey(newBeeMallShoppingCartItem.getGoodsId())) {
                    ...
                    String info = newBeeMallGoodsTemp.getGoodsIntro();
                    // 字符串过长导致文字超出的问题
                    if (info.length() > 45) {
                    ...
                    //获取一级分类名称
                    String yjName;
                    long id = newBeeMallGoodsTemp.getGoodsCategoryId();
                    GoodsCategory temp;
                    for(int i=0;i<5;i++){
                        temp = goodsCategoryMapper1.selectByPrimaryKey(id);
                        if (temp==null){
                            newBeeMallShoppingCartItemVO.setYjName("未找到分类");
                            break;
                        }
                        if(temp.getParentId()==0){
                            newBeeMallShoppingCartItemVO.setYjName(temp.getCategoryName());
                            break;
                        }else {
                            id = temp.getParentId();
                        }
                    ...
                    newBeeMallShoppingCartItemVO.setTag(newBeeMallGoodsTemp.getTag());
                    newBeeMallShoppingCartItemVO.setGoodsIntro(info);
